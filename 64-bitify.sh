...
options=("Install 64-bit kernal code (do this first!)" "Enable 64-bit kernal" "Disable 64-bit kernal" "Update script" "Revert config.txt")
...
select opt in "${options[@]}" "Quit"; do 
    ...
    4 ) 
    5 ) sudo mv /boot/config.txt.bak /boot/config.txt && echo "Reverted config.txt to when 64-bit was installed"
